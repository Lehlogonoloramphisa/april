...
class StripeController extends Controller
{
    ...
    class StripeController extends Controller
{
    public function getPaymentSuccessStatus(Request $request)
    {
        $paymentSuccess = $request->session()->get('payment_success', false);
        return response()->json(['payment_success' => $paymentSuccess]);
    }
}
    ...
    public function stripeCheckoutSuccess(ValidatorRequest $request)
    {
        ...
        if ($payment_for == "wallet") {
            ...
            $payment_success = true; // Set payment_success to true as payment is successful for wallet
        } else {
            // Handle other payment cases if any
        }
    
        return view('success', compact('web_booking_value', 'request_id'))->with('payment_success', $payment_success);
    }
